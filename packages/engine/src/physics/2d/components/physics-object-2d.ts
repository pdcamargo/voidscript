/**
 * PhysicsObject2D Component
 *
 * Runtime marker component for entities synchronized to Rapier 2D physics.
 ...
 * - Allows querying: .none(PhysicsObject2D) for new entities, .all(PhysicsObject2D) for synced
 *
 * Lifecycle:
 * - Created by physics component sync system when RigidBody2D/Collider2D is added
...
export interface PhysicsObject2DData {
  /** Rapier rigid body handle */
  bodyHandle: number;

  /** Rapier collider handle (if entity has Collider2D) */
  colliderHandle?: number;

  // ============================================================================
  // Character Controller Fields (only present if entity has CharacterController2D)
  // ============================================================================

  /** Rapier character controller handle (only if CharacterController2D component exists) */
  controllerHandle?: number;

  /** Actual movement computed by controller after collision resolution */
  computedMovement?: THREE.Vector2;
 ...
 */
export const PhysicsObject2D = component<PhysicsObject2DData>(
  'PhysicsObject2D',
